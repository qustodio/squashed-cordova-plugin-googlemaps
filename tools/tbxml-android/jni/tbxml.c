...
jlongArray Java_za_co_twyst_tbxml_TBXML_jniListElementsForQuery(JNIEnv* env,jobject object,jlong document,jlong element,jstring query) {
      ...
      if (doc) {
    	  if (node) {
    		  size_t queryLength = strlen(str);

    		  mutableQuery = (char *) malloc(queryLength + 1);
    		  ...
    		  memcpy(mutableQuery,str,queryLength + 1);

    		  char *token = strtok(mutableQuery,".");
